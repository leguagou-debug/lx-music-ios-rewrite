...
const SettingRow: React.FC<SettingRowProps> = ({ label, value, onPress, children }) => (
  <TouchableOpacity style={styles.row} activeOpacity={onPress ? 0.6 : 1} onPress={onPress} disabled={!onPress}>
    ...
    {children ?? (value ? <Text style={styles.rowValue}>{value}</Text> : null)}
  </TouchableOpacity>
)
...
const Settings: React.FC<{ componentId: string }> = ({ componentId }) => {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Section title="列表">
        <SettingRow label="显示歌曲来源">
          <Switch value={setting['list.isShowSource']} onValueChange={v => set('list.isShowSource', v)} trackColor={{ true: '#07c556' }} />
        </SettingRow>
        <SettingRow label="显示专辑名">
          <Switch value={setting['list.isShowAlbumName']} onValueChange={v => set('list.isShowAlbumName', v)} trackColor={{ true: '#07c556' }} />
        </SettingRow>
        <SettingRow label="显示歌曲时长">
          <Switch value={setting['list.isShowInterval']} onValueChange={v => set('list.isShowInterval', v)} trackColor={{ true: '#07c556' }} />
        </SettingRow>
      </Section>
